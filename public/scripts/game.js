...
class Jugador {
  constructor(nombre, numJugador) {
    ...
    this.puntuacion = 0;
    // metodo para incrementar puntuacion al marcar una casilla
  }
  sumarPunto() {
    this.puntuacion += 1;
  }
  porcentajeConquistado() {
    return (100 * this.puntuacion) / 36;
  }
}
...
function junto(x, y, jugador) {
  ...
  if (jugador.nombre == jugador_actual.nombre) {
    //El jugador que está jugando la partida es igual al jugador actual (COMPROBACIÓN)
    cuadrados.forEach((element) => {
      //Recorremos todos los elementos de nuestra lista cuadrados
      if (element.color == jugador.color) {
        if (
          (x - tamanyoCuad == element.x && y == element.y) ||
          (x - tamanyoCuad == element.x && y + tamanyoCuad == element.y) ||
          (x == element.x && y + tamanyoCuad == element.y) ||
          (x + tamanyoCuad == element.x && y + tamanyoCuad == element.y) ||
          (x + tamanyoCuad == element.x && y == element.y) ||
          (x + tamanyoCuad == element.x && y - tamanyoCuad == element.y) ||
          (x == element.x && y - tamanyoCuad == element.y) ||
...
function crearTablero() {
  ...
  for (let i = 0; i < 6; i++) {
    //creamos fila y abajo en j crea las casillas de la fila, después vuelve a i suma 1 y vuelve ha hacer todas las casillas j de la nueva fila
    let fila = i;
    for (let j = 0; j < 6; j++) {
  ...
  canvas.addEventListener("click", (event) => {
    ...
    cuadrados.forEach((element) => {
      if (
        x > element.x &&
        x < element.x + tamanyoCuad &&
        y > element.y &&
        y < element.y + tamanyoCuad
      ) {
        //comprobamos la ubicación del ratón
        if (element.color == "transparent") {
          if (jugador_actual.puntuacion == 0) {
            //que el jugador no haya marcado ninguna casilla
            let newCuadrado = new Cuadrado(
              element.x,
              element.y,
              tamanyoCuad,
              jugador_actual.color
            ...
            jugador_actual.sumarPunto();
            /* Actualizar el valor en la vista */
            document.getElementById("punt_jugador_local").innerHTML =
              jugador_actual.puntuacion;
          } else if (jugador_actual.puntuacion > 0) {
            //el jugador ya ha marcaddo una casilla
            if (junto(element.x, element.y, jugador_actual) == true) {
              ...
              /* Actualizar el valor en la vista */
              document.getElementById("punt_jugador_local").innerHTML =
                jugador_actual.porcentajeConquistado().toFixed(2);
            }
          }
        }
      }
